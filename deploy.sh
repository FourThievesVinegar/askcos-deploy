...
pull-images() {
  if [ "$LOCAL" = "false" ]; then
    docker pull ${ASKCOS_SITE_IMAGE_REGISTRY}askcos-site:${VERSION_NUMBER}
  fi
}

get-image-date() {
  # Retrieve image build date for website footer
  UPDATE_DATE=$(docker inspect -f '{{ .Created }}' ${ASKCOS_SITE_IMAGE_REGISTRY}askcos-site:${VERSION_NUMBER})
